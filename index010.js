...
function objectView(object){
  ...
  // keyArray : person_1.age
  //return keyArray[1]; dev tool > console 테스트용
  for(let i = 0; i < keyArray.length;i++){ 
    if (object[keyArray[i]] == `[object Object]`){
      console.log(`<--- ${keyArray[i]} --- `);
      
      objectView(object[keyArray[i]]);
      console.log(`--- ${keyArray[i]} --->`);
    }
    else{
      console.log(`${keyArray[i]} : ${object[keyArray[i]]} : ${typeof object[keyArray[i]]}`);
    }
  }
}
